...
public class GPIO64EndPoints
    ...
    public SimpleMessage outputPin(@ParamProp(name = "gpio") String gpio,
        ...
        try {
            GPIO64Tools.SINGLETON.setOutputPin(pin, DigitalState.getState(state), duration);
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    public SimpleMessage outputPWM(@ParamProp(name = "gpio") String gpio,
        ...
        GPIO64Tools.SINGLETON.setPWM(pin, freq, dutyCycle, delay, count);
    ...
    public void configPWM(@ParamProp(name = "range") int range) {
        GPIO64Tools.SINGLETON.setPWMRangeMod(range, -1);
    }
    ...
    public NVGenericMap pwmConfig() {
        ...
        ret.add(new NVInt("pwm_max_steps", GPIO64Tools.SINGLETON.getPWMRange()));
        ret.add(new NVPair("pwm_range", GPIO64Tools.PWM_RANGE.toString()));
