...
public class GPSParser {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (tokens.length == 12) {
                switch (i) {
                    ...
                    case 9:
                        if (tokens[i] != null) {
                            LocalDateTime ldt = LocalDateTime.parse(tokens[9] + tokens[1], DateFormatter.GPS_DATE_FORMAT);
                            navig.setGPSTime(ldt.toInstant(ZoneOffset.UTC).toEpochMilli());
                        }
                        break;
                    case 10:
                        break;
    ...
    public static GPSDateTime parseGPZDA(String[] tokens, String checkSum) throws ParseException {
        ...
        ZonedDateTime zdt = ZonedDateTime.parse(toParse + timeZone, DateFormatter.GPS_ZDA_FORMAT);
        date.setGPSTime(zdt.toInstant().toEpochMilli());
