			...
			for (; index < args.length; index++) {
				...
				switch (action) {
					...
					case MONITOR:
						...
						GPIOMonitorConfig gm = new GPIOMonitorConfig().setMonitor(gpioPin).setFollowers(toSet.toArray(new GPIOPin[0])).setFollowersHighDelay("6sec").setFollowersLowDelay("0sec").name(gpioPin.toString());
						System.out.println(GSONUtil.DEFAULT_GSON.toJson(gm));
						input.addListener(new PinStateMonitor(gm, null, true));
