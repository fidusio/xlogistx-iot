...
public class NGPIOEndPoints
        extends PropertyContainer<NVGenericMap> {

    private static final Logger log = Logger.getLogger(NGPIOEndPoints.class.getName());
    ...
    public SimpleMessage outputPin(@ParamProp(name = "gpio") String gpio,
        ...
        NGPIOPin pin = NGPIOPin.lookupGPIO(gpio);
        long duration = 0;
        if (durationParam != null) {
            duration = Const.TimeInMillis.toMillis(durationParam);
        }

        NGPIOTools.SINGLETON.setOutputPin(pin, DigitalState.getState(state), duration);
    ...
    //@ParamProp(name="monitor", optional = true) boolean monitor)
    {
        ...
        NGPIOTools.SINGLETON.setPWM(pin, freq, dutyCycle, Const.TimeInMillis.toMillis(duration));
    ...
    public SimpleMessage outputPWM(@ParamProp(name = "gpio") String gpio,
        ...
        NGPIOTools.SINGLETON.setPWM(pin, freq, dutyCycle, delay, count);
    ...
    public void configPWM(@ParamProp(name = "range") int range) {
        NGPIOTools.SINGLETON.setPWMRangeMod(range, -1);
    }
    ...
    public NVGenericMap pwmConfig() {
        ...
        ret.add(new NVInt("pwm_max_steps", NGPIOTools.SINGLETON.getPWMRange()));
        ret.add(new NVPair("pwm_range", NGPIOTools.PWM_RANGE.toString()));
    ...
    public SimpleMessage mapGPIO(@ParamProp(name = "gpio") String gpio,
        ...
        NGPIOPin gpioPin = NGPIOPin.lookupGPIO(gpio);
        NGPIOPin.mapGPIOName(name, gpioPin);
    ...
    public SimpleMessage unmapGPIO(@ParamProp(name = "name") String name) {
        NGPIOPin gpioPin = NGPIOPin.unmapGPIOName(name);
        if (gpioPin == null)
        ...
        if (getProperties() != null) {
            ...
            if (gpiosMap != null) {
                for (GetNameValue<?> pinInfo : gpiosMap.values()) {
                    try {
                        NGPIOPin gpio = NGPIOPin.mapGPIOName(pinInfo.getName(), "" + pinInfo.getValue());
            ...
            if (gpiosInit != null) {
                for (GetNameValue<?> gnv : gpiosInit.values()) {
                    try {
                        ...
                        NGPIOPin pin = NGPIOPin.lookupGPIO(pinConfig.getName());
                        ...
                        if (pin != null) {
                            ...
                            NGPIOTools.SINGLETON.setOutputPin(pin.getValue(), DigitalState.getState(state), duration);
