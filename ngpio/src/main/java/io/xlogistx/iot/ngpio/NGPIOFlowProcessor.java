 ...
 */
public class NGPIOFlowProcessor
        extends DefaultFlowProcessor<DigitalStateChangeEvent> {

    private static final transient Logger log = Logger.getLogger(NGPIOFlowProcessor.class.getName());
    private NPinStateMonitorConfig pinStateMonitorConfig;

    public NGPIOFlowProcessor(NPinStateMonitorConfig pinStateMonitorConfig, TaskSchedulerProcessor tsp) {
        super(tsp);
    ...
    public synchronized void accept(FlowEvent<DigitalStateChangeEvent> event) {
        NPinStateMonitor source = (NPinStateMonitor) event.getSource();
        ...
        if (source == pinStateMonitorConfig.getMaster()) {
            ...
            for (NPinStateMonitor psm : pinStateMonitorConfig.getSlaves()) {
                psm.setEnabled(state == DigitalState.LOW);
            }
        } else {
            if (source.isEnabled()) {
                DigitalState state = event.getFlowType().state();
                NGPIOConfig gpiom = source.getMonitorConfig();
    ...
    public synchronized void init() {
        DigitalState masterState = NGPIOTools.SINGLETON.getPinState(pinStateMonitorConfig.getMaster().getMonitorConfig().getToMonitor());
        for (NPinStateMonitor psm : pinStateMonitorConfig.getSlaves()) {
            psm.flowProcessorSetter(this);
            psm.setEnabled(masterState == DigitalState.LOW);
            DigitalInput input = NGPIOTools.SINGLETON.setInputPin(psm.getMonitorConfig().getToMonitor());
            input.addListener(psm);
        }

        pinStateMonitorConfig.getMaster().flowProcessorSetter(this);
        DigitalInput input = NGPIOTools.SINGLETON.setInputPin(pinStateMonitorConfig.getMaster().getMonitorConfig().getToMonitor());
